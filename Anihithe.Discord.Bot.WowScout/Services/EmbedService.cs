...
public class EmbedService
{
    public Embed SimpleEmbed(DtoRoot character)
    {
        var embed = new EmbedBuilder()
            ...
            .AddField("Status",$"{(character.Expansions![0].Instances[0].Modes[0].Progress.Encounters[0].CompletedCount >=1 ? "Killed" : "Alive")}", true);
    ...
    public Embed NoSoSimpleEmbed(DtoRoot character)
    {
        ...
        foreach (var encounter in character.Expansions![randExp].Instances[randInst].Modes[randMode].Progress.Encounters)
        {
            bossList.AppendLine(encounter.Encounter.Name);
            bossStatus.AppendLine($"{(encounter.CompletedCount == 0 ? "Alive" : "Killed")} ({encounter.CompletedCount} times)");
            lastkill.AppendLine($"{DateTimeOffset.FromUnixTimeMilliseconds((long) encounter.LastKillTimestamp).ToLocalTime()}");
        }
